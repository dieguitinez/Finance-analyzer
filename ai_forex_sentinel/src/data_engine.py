...
class DataEngine:
    ...
    def fetch_dxy_data(self, interval="1h", period="20d"):
        """
        Fetches the US Dollar Index (DXY).
        Used for Macro Institutional Correlation checks.
        """
        return self._fetch_yahoo_ticker("DX-Y.NYB", interval, period)

    def fetch_macro_data(self, interval="1h", period="20d"):
        """
        Fetches US 10-Year Treasury Yields and Gold.
        """
        yields = self._fetch_yahoo_ticker("^TNX", interval, period)
        gold = self._fetch_yahoo_ticker("GC=F", interval, period)
        return {"yields": yields, "gold": gold}

    def _fetch_yahoo_ticker(self, ticker, interval, period):
        try:
            ...
            print(f"[DataEngine] Error fetching {ticker}: {e}")
            return None
...
class FundamentalEngine:
    ...
    def get_pair_sentiment(pair_name):
        ...
        try:
            ...
            if not GENAI_READY or not api_key:
                # V6 FALLBACK: Use Vader Sentiment (Local & Fast)
                print("[FundamentalEngine] Gemini not ready or key missing. Using VADER fallback...")
                from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer
                analyzer = SentimentIntensityAnalyzer()
                total_compound = 0
                for item in news_items:
                    vs = analyzer.polarity_scores(item['title'])
                    item['score'] = vs['compound']
                    total_compound += vs['compound']
                
                avg_compound = total_compound / len(news_items) if news_items else 0
                # Map -1..1 to 0..100
                final_score = (avg_compound + 1) * 50
