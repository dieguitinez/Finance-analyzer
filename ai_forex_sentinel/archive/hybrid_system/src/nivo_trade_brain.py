...
class NivoTradeBrain:
    ...
    def analyze_market(self):
        ...
        # VETO THRESHOLD: We require at least 75% conviction on the legacy metrics
        # Long: 50 + (75/2) = 87.5
        # Short: 50 - (75/2) = 12.5
        VETO_LONG_THRESHOLD = 87.5
        VETO_SHORT_THRESHOLD = 12.5
        
        if is_long and final_score >= VETO_LONG_THRESHOLD:
            signal = "BUY"
            reasons.append(f"🔥 Legacy Convicton: {percentage_score:.1f}% (Breakout Validated)")
            ...
            signal = "WAIT"
            reasons.append(f"🛡️ FALSE BREAKOUT VETO: Legacy Conviction only {percentage_score:.1f}%. Need 75%. Trade Muted.")
