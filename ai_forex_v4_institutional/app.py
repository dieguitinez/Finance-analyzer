...
# Enhanced Lite Mode: Checks for missing libs OR explicit environment flag
LITE_MODE = not (TORCH_AVAILABLE and HMM_AVAILABLE) or os.environ.get('ST_LITE', 'False').lower() == 'true'
...
def load_models():
    ...
    """
    if LITE_MODE:
        # In Lite Mode, we return the classes but they will use internal mocks if imports fail
...
def fetch_seasonality_data(pair_name):
    """ Fetches seasonality data (Cached to prevent memory spikes) """
    df = DataEngine().fetch_data(DataEngine.get_symbol_map(pair_name), "1d", period="5y")
    if df is not None:
        df['Return'] = df['Close'].pct_change() * 100
        df['Month'] = df.index.month
    return df

# Main sequential executor preloading all tabs to prevent Out of Memory
with st.spinner("⚡ Synchronizing Nivo Datasets (Optimized)..."):
    data, current_fetch_ts = fetch_market_data(selected_pair_ui, selected_tf, oanda_token, oanda_id)
    news, avg_s = fetch_news_data(selected_pair_ui)
    s_data = fetch_seasonality_data(selected_pair_ui)
    global_results = fetch_global_scan(pair_options, selected_tf, oanda_token, oanda_id)
        ...
        if prompt := st.chat_input(t['kai_placeholder']):
            ...
            with chat_container.chat_message("assistant"):
                ...
                model_cascade = [
                    ...
                    'gemini-3.1-flash-lite',  # Ultra-fast backup
                    'gemini-2.5-pro',         # Intelligence backup
                    'gemini-2.0-flash',       # Final emergency backup
                ]
                response_text = None
