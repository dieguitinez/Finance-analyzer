...
def run_headless_cycle(pairs_list: list | None = None):
    """
    ...
    # 1. Macro Awareness Check (V4)
    logger.info("🌍 Checking Global Institutional Macro context...")
    engine = DataEngine(oanda_config={"token": _token, "account_id": _account})
    macro_data = engine.fetch_macro_data() # Fetch Yields and Gold
    dxy_df = engine.fetch_dxy_data()
    
    macro_veto_global = False
    macro_bias_usd = "NEUTRAL"
    
    if dxy_df is not None and len(dxy_df) > 20:
        dxy_close = dxy_df['Close'].iloc[-1]
        dxy_ema = dxy_df['Close'].ewm(span=20, adjust=False).mean().iloc[-1]
        if dxy_close > dxy_ema * 1.002: # 0.2% above EMA = Strong Bullish Bias
            macro_bias_usd = "STRONG_BULLISH"
            logger.info("💹 [MACO BIAS] USD is Strong Bullish based on DXY.")
        elif dxy_close < dxy_ema * 0.998:
            macro_bias_usd = "STRONG_BEARISH"
            logger.info("💹 [MACO BIAS] USD is Strong Bearish based on DXY.")
    ...
    for oanda_symbol in pairs_list:
        ...
        try:
            ...
            try:
                report_data = {
                    "pair": pair,
                    "price": float(f"{float(df['Close'].iloc[-1]):.5f}"),
                    "brain_signal": raw_signal,
                    "hmm_regime": hmm_lbl,
                    "lstm_prob": float(f"{float(lstm_prob):.2f}"),
            ...
            if oanda_symbol in _open_instruments:
                ...
                # HYBRID FINAL SAFETY LAYER (Relaxed V5 for Frequency)
                _lp = float(lstm_prob)
                _ss = float(sentiment_score)
                
                # Simplified robust logic: Only veto if there is EXPLICIT contradiction
                # If Market is Neutral (50/50), we proceed.
                pass_hybrid = True
                
                if raw_signal == "BUY":
                    if _lp < 49.0: pass_hybrid = False # LSTM says Sell strongly
                    if _ss < 45.0: pass_hybrid = False # News is explicitly bearish
                elif raw_signal == "SELL":
                    if _lp > 51.0: pass_hybrid = False # LSTM says Buy strongly
                    if _ss > 55.0: pass_hybrid = False # News is explicitly bullish
                
                # V4 INSTITUTIONAL MACRO VETO
                is_usd_base = pair.startswith("USD/")
                is_usd_quote = pair.endswith("/USD")
                
                if macro_bias_usd == "STRONG_BULLISH":
                    if is_usd_quote and raw_signal == "BUY":
                        logger.warning(f"[VETO] Se bloqueó COMPRA en {pair} por fortaleza de USD (DXY).")
                        record_decision(pair, "WAIT", hmm_lbl, _lp, _ss, dom_outlook, reason="Veto Macro: USD Fuerte")
                        continue
                    if is_usd_base and raw_signal == "SELL":
                        logger.warning(f"[VETO] Se bloqueó VENTA en {pair} por fortaleza de USD (DXY).")
                        record_decision(pair, "WAIT", hmm_lbl, _lp, _ss, dom_outlook, reason="Veto Macro: USD Fuerte")
                        continue

                if not pass_hybrid:
                    logger.info(f"🛡️ [HYBRID VETO] Alerta de Contradicción: IA({_lp}) | NEWS({_ss}). Señal abortada para {pair}.")
                    record_decision(pair, "WAIT", hmm_lbl, _lp, _ss, dom_outlook, reason="Veto Híbrido")
                    continue
                ...
                try:
                    ...
                    _units_val = float(units)
                    trade_units = _units_val if raw_signal == "BUY" else -abs(_units_val)
                    ...
                    if _tg_token and _tg_chat:
                        NotificationManager.trade_signal_alert(pair=pair, signal=raw_signal, score=int(float(sentiment_score)), weight=1.0, direction=float(lstm_prob), 
            ...
            try:
                ...
                if 'brain_analysis' in locals(): del brain_analysis
                if 'brain' in locals(): del brain
                if 'cortex' in locals(): del cortex
                import gc
                gc.collect()
...
if __name__ == "__main__":
    ...
    if args.diagnostic:
        try:
            ...
            print(json.dumps({
                "pair": p, 
                "price": float(f"{float(d['Close'].iloc[-1]):.5f}"), 
                "signal": an.get("signal"),
                "hmm": hl, 
                "lstm": float(f"{float(lp):.2f}"), 
                "news": fs
            }))
        except Exception as e:
