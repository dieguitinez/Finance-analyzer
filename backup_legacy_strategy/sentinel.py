...
def run_infinite_sentinel():
    ...
    while True:
        ...
        if not is_market_open():
            logger.info("Market is CLOSED. Sentinel in standby.")
            time.sleep(900)
            continue
        ...
        for asset in watch_assets:
            ...
            # Symmetric Execution Check (Strict ±30 Rule)
            is_strong_signal = (signal == "BUY" and score >= 80) or (signal == "SELL" and score <= 20)
            
            if signal != "WAIT" and not veto and is_strong_signal:
