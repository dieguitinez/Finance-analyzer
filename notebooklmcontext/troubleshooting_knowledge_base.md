...
### 📉 Legacy & Resource Efficiency (Context: 1GB RAM VM)
- **Historical Issue**: `/status` command in Telegram initially crashed the bot in low-RAM cloud VMs.
- **Hardware Upgrade**: The system now runs on a **Physical Linux Hub (8GB RAM / Ryzen 5)**, removing memory-bound crashes.
- **Best Practice**: We still maintain the **"Scan First, Detail Later"** logic in `quantum_engine/nivo_tg_bot.py`. It lists open positions with one API call first, then fetches details *only* for active trades. This minimizes network latency regardless of RAM.
