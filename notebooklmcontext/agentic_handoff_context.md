...
- **Hardware Specs**: The production environment is a **Physical HP ENVY Laptop** running Linux with **8GB RAM** and a **Ryzen 5 (8-core)** CPU. This provides significant headroom for dual-engine operations (Forex + Stocks).
- **Optimization**: `/status` still prioritizes active symbol iteration for speed, but memory constraints are virtually non-existent on this hardware.
