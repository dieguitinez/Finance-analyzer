...
class NivoCortex:
    ...
    def evaluate_veto(self, df: pd.DataFrame = None):
        ...
        if regime == "CRASH_MODE":
            # MULTIMODAL OVERRIDE: If HMM says crash, but LSTM (Neural Vector) is with high conviction, we bypass.
            # This allows the bot to catch "Panic Bottoms" or "Sharp Reversals".
            if prob > 70:
                veto = False
                reason = "BYPASS: HMM CRASH detecado pero LSTM indica REBOTE Violento (Prob > 70%). Procediendo con cautela."
            elif prob < 30:
                veto = False
                reason = "BYPASS: HMM CRASH detectado pero LSTM indica CAÍDA Parabólica (Prob < 30%). Procediendo con cautela."
            else:
                veto = True
