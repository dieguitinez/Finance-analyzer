    ...
    for oanda_symbol in pairs_list:
        ...
        try:
            ...
            if oanda_symbol in _open_instruments:
                ...
                # HYBRID FINAL SAFETY LAYER (Relaxed for Frequency)
                _lp = float(lstm_prob)
                _ss = float(sentiment_score)
                ai_agreement = (raw_signal == "BUY" and _lp >= 52.0) or (raw_signal == "SELL" and _lp <= 48.0)
                news_agreement = (raw_signal == "BUY" and _ss >= 51.0) or (raw_signal == "SELL" and _ss <= 49.0)

                if not (ai_agreement and news_agreement):
                    logger.info(f"🛡️ [HYBRID VETO] IA:{ai_agreement} ({_lp}) | NEWS:{news_agreement} ({_ss}). Señal abortada para {pair}.")
