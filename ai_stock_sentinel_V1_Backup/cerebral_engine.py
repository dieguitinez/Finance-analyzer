    ...
    def analyze_momentum(self, df, symbol=""):
        ...
        if is_whale_present:
            if is_bullish and is_not_overbought:
                return "BUY", f"🚀 TREND FOLLOWING: Confirmación alcista con Volumen Institucional ({current_volume/avg_volume:.1f}x) y RSI sano ({current_rsi:.1f})."
            
            if is_bearish and is_not_oversold:
                # Nota: El watcher bloqueará esto si no tenemos posición (No Shorts)
                return "SELL", f"📉 TREND BREAK: Debilitamiento detectado con Volumen ({current_volume/avg_volume:.1f}x) y RSI ({current_rsi:.1f})."

        return None, f"😴 NEUTRAL: Sin volumen institucional significativo ({current_volume/avg_volume:.1f}x) o tendencia clara."
